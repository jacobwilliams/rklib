...
!  Integrate a two-body orbit around the Earth.
    ...
    allocate(rktvd2_class :: s);   allocate(s2, source=s); call run_all_tests('rktvd2'  ,[235, 195, 52]); call finish()
    allocate(rk3_class :: s);      allocate(s2, source=s); call run_all_tests('rk3'     ,[220, 235, 52]); call finish()
    allocate(rktvd3_class :: s);   allocate(s2, source=s); call run_all_tests('rktvd3'  ,[220, 255, 52]); call finish()
